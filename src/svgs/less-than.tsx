const LessThan = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    width={60}
    height={60}
    fill="none"
  >
    <path fill="url(#lessThan)" d="M0 0h60v60H0z" />
    <defs>
      <pattern
        id="lessThan"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#less-than" transform="scale(.01111)" />
      </pattern>
      <image
        xlinkHref="data:image/png;base64,"
        id="less-than"
        width={90}
        height={90}
      />
    </defs>
  </svg>
);
export default LessThan;
